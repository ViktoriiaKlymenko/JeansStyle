...
namespace JeansStyle.WEB.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> GetProductCard([FromQuery] string searchRequest)
        {
            if (ModelState.IsValid)
            {
                var products = await _searchService.GetProductsByTitleAndDescription(searchRequest);
                if (products.Products.Count == 0)
                {
                    ViewBag.Request = searchRequest;
                    return View("NotFound");
